...
import { MYFrameModifier } from "../modifiers/layout/FrameModifier";
import { MYOnAppearModifier } from "../modifiers/lifecycle/OnAppearModifier";
import { MYAllowsHitTestingModifier } from "../modifiers/interaction/AllowsHitTestingModifier";
import { MYOnChangeModifier } from "../modifiers/lifecycle/OnChangeModifier";
import { MYBackgroundModifier, MYBackgroundType } from "../modifiers/styling/BackgroundModifier";
...
import { MYOffsetModifier } from "../modifiers/layout/OffsetModifier";
import { MYOnDisappearModifier } from "../modifiers/lifecycle/OnDisappearModifier";
import { MYOpacity } from "../types/Opacity";
import { MYOpacityModifier } from "../modifiers/styling/OpacityModifier";
import { MYPadding } from "../types/Padding";
import { MYPaddingModifier } from "../modifiers/layout/PaddingModifier";
...
import { MYForegroundStyleModifier } from "../modifiers/styling/ForegroundStyleModifier";
import { MYDisabledModifier } from "../modifiers/interaction/DisabledModifier";
import { MYContextWrapper } from "../react-bridge/ContextWrapper";
import { MYStateObserver } from "../state/ReactiveSystem";

const MYCompositeWrapper: React.FC<{ view: MYView, frame?: MYFrame }> = ({ view, frame }) => {
    const [, forceRender] = React.useReducer((x) => x + 1, 0);

    const observerRef = React.useRef<MYStateObserver | null>(null);
    if (!observerRef.current) {
        observerRef.current = new MYStateObserver(forceRender);
    }

    React.useEffect(() => {
        return () => observerRef.current?.cleanup();
    }, []);

    const childView = observerRef.current.track(() => view.body());

    return childView.makeView(frame);
};
...
export abstract class MYView {
    ...
    makeView(frame?: MYFrame): React.ReactNode {
        if (this.body === MYView.prototype.body) {
            throw new Error("Primitive views must override makeView(). Composite views must override body().");
        }
        return <MYCompositeWrapper view={this} frame={frame} key={this.viewId} />;
    }
